...
init_environment() {
    ...
    # Install tmux configuration to user home directory (kept for the session-picker path)
    ...
    # Seed herdr config to the persistent XDG location on first run only,
    # so the baked-in defaults apply but user edits survive restarts.
...
get_claude_launch_command() {
    ...
    if [ "$auto_launch_claude" = "true" ]; then
        # Use herdr for session persistence + agent-aware workspaces/panes.
        # herdr-launch.sh attaches the persistent session and ensures a single
        # persistent "parent" Claude agent (replaces tmux new-session -A -s claude 'claude').
        echo "${welcome_prefix}/opt/scripts/herdr-launch.sh"
    else
